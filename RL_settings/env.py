...
class EnvOffline(gym.Env):
    ...
    def __init__(self, list_pred_sc, list_thresholds,list_gtruth, model_path="C:/Users/gjh/Desktop/RLMSAD-master/base_detectors"):

        # Length of the testing data, number of models 测试数据长度、模型数量
        self.len_data = len(list_pred_sc[0])
        self.num_models = len(list_pred_sc)

        #List of ground truth labels  地面真相标签列表
        self.gtruth = list_gtruth
        
        # Get the list of pretrained models  获取预训练模型的列表
        self.model_path = model_path 
        self.model_list = [f for f in os.listdir(self.model_path) if f.endswith('.sav')]

        # Extract the model names 提取模型名称
        self.model_names = [f.split('_')[0] for f in self.model_list]
        
        # Raw scores and thresholds of the testing data 测试数据的原始分数和阈值
        self.list_pred_sc = list_pred_sc
        self.list_thresholds = list_thresholds

        # Scale the raw scores/thresholds and save each scaler 缩放原始分数/阈值并保存每个缩放器
        self.scaler = []
        self.list_scaled_sc = []
        ...
        # Extract predictions  提取预测   每个模型一个预测值
        self.list_pred = []
        for i in range(self.num_models):
        ...
        # Extract prediction-concensus confidence  提取预测置信度
        ...
        # Extract distance-to-threshold confidence  提取距离阈值置信度  增加的绝对值
        self.dist_conf=[]
        for length in range(self.len_data):
            dist_tmp = []
            for i in range(self.num_models):
                dist_tmp.append(abs(self.list_scaled_sc[i][length] - self.list_scaled_thresholds[i]))
            self.dist_conf.append(dist_tmp)
        

        # Gym settings
        self.action_space = spaces.Discrete(self.num_models)    # 动作空间
        # state_dim is 5 , each corresponds to scaled_sc, scaled_thresholds, pred, concensus_conf, dist_conf 
        self.observation_space = spaces.Box(low=0, high=1, shape=(5, ), dtype=np.float32)   # 状态空间
        self.seed()
        self.reset()
...
def eval_model(model,env):
    ...
    while True:
        ...
        if done:
            break
    acc = accuracy_score(gtruth, preds)
...
class EnvOffline_consensus_conf(gym.Env):
    ...
    def __init__(self, list_pred_sc, list_thresholds,list_gtruth, model_path="C:/Users/gjh/Desktop/RLMSAD-master/base_detectors"):
        ...
        # Raw scores and thresholds of the testing data  提取模型名称
        self.list_pred_sc = list_pred_sc
        self.list_thresholds = list_thresholds
        ...
        # Extract prediction-concensus confidence  提取预测集中置信度
        self.list_concensus_conf = [] # how many models have predicted 1 (anomaly)
        for length in range(self.len_data):
            num_a_tmp=0 # number of models have predicted 1 (anomaly)
            for i in range(self.num_models):
...
class EnvOffline_dist_conf(gym.Env):
    ...
    def __init__(self, list_pred_sc, list_thresholds,list_gtruth, model_path="C:/Users/gjh/Desktop/RLMSAD-master/base_detectors"):
        ...
        # Extract distance-to-threshold confidence  提取距离阈值置信度
        self.dist_conf=[]
        for length in range(self.len_data):
            dist_tmp = []
            for i in range(self.num_models):
...
class TrainEnvOffline_dist_conf(EnvOffline_dist_conf):
    ...
    def step(self, action):
        ...
        # Check whether the episode is over 检查事件是否结束
        if self.pointer >= self.len_data:
            self.done = True
        else:
            self.done = False
...
class TrainEnvOffline_None(EnvOffline_None):
    '''The training environment in offline setting.
    ...
    def _get_state(self, action=None):
        ...
        # Get the current state
        observation = np.zeros(3)  # 4 dims - scaled scores, scaled thresholds, labels, dist_conf
        observation[0] = self.list_scaled_sc[action][self.pointer]
        observation[1] = self.list_scaled_thresholds[action]
        observation[2] = self.list_pred[action][self.pointer]
        # observation[3] = self.dist_conf[self.pointer][action]
